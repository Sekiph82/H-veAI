...
export function AuditCenterPage() {
  ...
  const startAudit = async (priorAuditId: string | null = null) => {
    ...
    try {
      ...
      setAudits((current) => [next, ...current.filter((audit) => audit.id !== next.id)]); setSelectedId(next.id); setNotice(next.modelStatus === "UNAVAILABLE" ? "Evidence collected. GPT audit provider is unavailable; no PASS was claimed." : "Audit completed with structured evidence.");
    }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { setLoading(false); }
  };
