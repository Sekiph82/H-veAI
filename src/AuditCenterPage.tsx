...
export function AuditCenterPage() {
  ...
  const startAudit = async (priorAuditId: string | null = null) => {
    ...
    try {
      ...
      setAudits((current) => [next, ...current.filter((audit) => audit.id !== next.id)]); setSelectedId(next.id); setNotice(next.state === "STALE" ? "Audit evidence became stale before persistence; re-run the audit." : next.modelStatus === "AVAILABLE" ? "Audit completed with structured evidence." : `Evidence persisted with model status ${next.modelStatus}; no authoritative model verdict was claimed.`);
    }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { setLoading(false); }
  };
